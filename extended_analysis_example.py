...
def main():
    ...
    print(
        f"  Detected doublets: {data.obs['predicted_doublet'].sum()} ({data.obs['predicted_doublet'].mean()*100:.1f}%)"
    )
    ...
    if (
        first_cluster in enrichment_results
        and len(enrichment_results[first_cluster]) > 0
    ):
        print(
            enrichment_results[first_cluster][
                ["gene_set", "overlap_size", "fold_enrichment", "pval_adj"]
            ].head()
        )
    ...
    if has_umap:
        ...
        vis.plot_umap(
            data,
            color=cluster_key,
            title="PBMC 3k - Clusters",
            save="pbmc_umap_clusters.png",
    ...
    print(f"\nDoublet detection:")
    print(
        f"  Predicted doublets: {data.obs['predicted_doublet'].sum()} ({data.obs['predicted_doublet'].mean()*100:.1f}%)"
    )
