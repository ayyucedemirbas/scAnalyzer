...
def harmony_integrate(
    ...
    for iteration in range(max_iter_harmony):
        ...
        dists = (
            np.sum(Z_corr**2, axis=1, keepdims=True)
        ...
        for k in range(n_clusters):
            ...
            for b in range(n_batches):
                ...
                Z_corr[batch_cluster_cells] += (
                    lamb * correction * weights.reshape(-1, 1)
                )
        ...
        if verbose:
            print(
                f"Harmony: Iteration {iteration+1}/{max_iter_harmony}, change = {change:.6f}"
            )
